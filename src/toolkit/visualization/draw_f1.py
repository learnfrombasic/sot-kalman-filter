...
def draw_f1(result, bold_name=None):
    ...
    for idx, (tracker_name, best_f1) in enumerate(
        sorted(best_f1.items(), key=lambda x: x[1], reverse=True)
    ):
        ...
        ax.plot(recall, precision, color=COLOR[idx], linestyle="-", label=label)
