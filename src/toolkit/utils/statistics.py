...
def calculate_failures(trajectory):
    ...
    failures = [
        i
        for i, x in zip(range(len(trajectory)), trajectory)
...
def calculate_accuracy(
    pred_trajectory, gt_trajectory, burnin=0, ignore_unknown=True, bound=None
):
    ...
    overlaps = region.vot_overlap_traj(
        pred_trajectory_[:min_len], gt_trajectory[:min_len], bound
    )
...
def determine_thresholds(scores, resolution=100):
    ...
    idxs = np.floor(
        np.linspace(delta - 1, len(scores) - delta, resolution - 2) + 0.5
    ).astype(np.int32)
