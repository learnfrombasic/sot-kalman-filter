...
class EAOBenchmark:
    ...
    def show_result(self, result, topk=10):
        ...
        if len(self.tags) == 1:
            ...
            print(bar)
            tracker_eao = sorted(
                result.items(), key=lambda x: x[1]["all"], reverse=True
            )[:topk]
            ...
            bar = "-" * len(header)
            formatter = (
                "{:^7.3f}|{:^15.3f}|{:^14.3f}|{:^15.3f}|{:^13.3f}|{:^11.3f}|{:^7.3f}|"
            )
            print(bar)
            print(header)
            print(bar)
            sorted_tacker = sorted(
                result.items(), key=lambda x: x[1]["all"], reverse=True
            )[:topk]
    ...
    def _calculate_eao(self, tracker_name, tags):
        ...
        for video in self.dataset:
            ...
            for tracker_traj in tracker_trajs:
                ...
                overlaps = calculate_accuracy(
                    tracker_traj, gt_traj, bound=(video.width - 1, video.height - 1)
                )[1]
        ...
        for tag in tags:
                ...
                if len(failures) > 0:
                    ...
                    for i in range(len(points)):
                        ...
                        if i != len(points) - 1:
                            ...
                            fweights[seg_counter] = (
                                seq_weight * w
                            )  # (len(fragment) / (traj_len-points[i]))
                        seg_counter += 1
                else:
                    # no failure
            ...
            eao_ = np.sum(expected_overlaps[is_valid] * weight[is_valid]) / np.sum(
                weight[is_valid]
            )
            eao[tag] = eao_
        return eao
