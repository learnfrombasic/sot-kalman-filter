...
class F1Benchmark:
    ...
    def _cal_precision_reall(self, tracker_name):
        ...
        for i in range(len(self.dataset)):
            ...
            if tracker_name not in video.pred_trajs:
                ...
                score = video.confidence[tracker_name]
            overlaps = calculate_accuracy(
                tracker_traj, gt_traj, bound=(video.width, video.height)
            )[1]
            f1[video.name], precision[video.name], recall[video.name] = calculate_f1(
                overlaps, score, (video.width, video.height), thresholds, N
            )
    ...
    def show_result(self, result, show_video_level=False, helight_threshold=0.5):
        ...
        for tracker_name, ret in result.items():
            ...
            sorted_tracker[tracker_name] = (
                precision[max_idx],
                recall[max_idx],
                f1[max_idx],
            )
        sorted_tracker_ = sorted(
            sorted_tracker.items(), key=lambda x: x[1][2], reverse=True
        )[:20]
