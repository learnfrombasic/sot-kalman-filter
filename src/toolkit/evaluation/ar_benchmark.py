...
class AccuracyRobustnessBenchmark:
    ...
    def show_result(
        self, result, eao_result=None, show_video_level=False, helight_threshold=0.5
    ):
        """pretty print result
        Args:
        ...
        if eao_result is not None:
            ...
            formatter = (
                ...
                + "}|{:^10.3f}|{:^12.3f}|{:^13.1f}|{:^7.3f}|"
            )
        else:
            header = "|{:^" + str(tracker_name_len) + "}|{:^10}|{:^12}|{:^13}|"
            header = header.format(
                "Tracker Name", "Accuracy", "Robustness", "Lost Number"
            )
            formatter = (
                "|{:^" + str(tracker_name_len) + "}|{:^10.3f}|{:^12.3f}|{:^13.1f}|"
            )
        bar = "-" * len(header)
        print(bar)
        print(header)
        ...
        if eao_result is not None:
            tracker_eao = sorted(
                eao_result.items(), key=lambda x: x[1]["all"], reverse=True
            )[:20]
        ...
        for i in range(len(self.dataset)):
            ...
            for tracker_traj in tracker_trajs:
                ...
                overlaps_ = calculate_accuracy(
                    tracker_traj, gt_traj, burnin=10, bound=(video.width, video.height)
                )[1]
